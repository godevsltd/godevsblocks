...
class Pagination extends BlockBase {
	...
	private function render_standard(
		...
		$links = paginate_links(
			array(
				'total'     => $total_pages,
				'current'   => $current,
				'prev_text' => $show_prev_next ? ( $prev_label ? $prev_label : __( '&laquo; Previous', 'godevs-blocks' ) ) : false,
				'next_text' => $show_prev_next ? ( $next_label ? $next_label : __( 'Next &raquo;', 'godevs-blocks' ) ) : false,
				'type'      => 'array',
		...
		return sprintf(
			...
			$classes,
			esc_attr__( 'Page navigation', 'godevs-blocks' ),
			$items
		);
	}
	...
	private function render_load_more(
		...
		return sprintf(
			...
			$total_pages,
			$current,
			esc_html( $label ? $label : __( 'Load More', 'godevs-blocks' ) )
		);
	}
