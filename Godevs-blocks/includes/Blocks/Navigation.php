	...
	public function render( array $attributes, string $content, WP_Block $block ): string {
		...
		if ( ! $menu_id ) {
			return sprintf(
				...
				$classes,
				$style_attr,
				esc_html__( 'Select a menu in block settings.', 'godevs-block' )
			);
		}
		...
		if ( $show_toggle ) {
			$toggle_html = '<button class="gb-navigation__toggle" aria-expanded="false" aria-label="'
				. esc_attr__( 'Toggle navigation', 'godevs-block' )
		...
		if ( ! $menu_html ) {
			$menu_html = '<ul class="gb-navigation__menu"><li>' . esc_html__( 'Menu not found.', 'godevs-block' ) . '</li></ul>';
		}
