		...
		switch ( $def['type'] ) {
			case 'integer':
				...
				if ( isset( $def['min'] ) && $int < $def['min'] ) {
					return new \WP_Error( 'goblocks_settings_range', esc_html__( 'Value below minimum.', 'godevs-blocks' ) );
				}
				if ( isset( $def['max'] ) && $int > $def['max'] ) {
					return new \WP_Error( 'goblocks_settings_range', esc_html__( 'Value above maximum.', 'godevs-blocks' ) );
				}
				return $int;
			...
			case 'string':
				...
				if ( isset( $def['enum'] ) && ! in_array( $str, $def['enum'], true ) ) {
					return new \WP_Error( 'goblocks_settings_enum', esc_html__( 'Invalid option value.', 'godevs-blocks' ) );
				}
				return $str;
