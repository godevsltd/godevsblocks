...
export function Edit( {
	...
	useEffect( () => {
		apiFetch< WpMenu[] >( {
			path: '/wp/v2/menus?per_page=100&_fields=id,name,slug',
		} )
			.then( ( data ) => {
	...
	useCssEngine( {
		...
		generatedCss,
		setAttributes: ( patch ) =>
			setAttributes( patch as Partial< NavigationBlockAttributes > ),
	} );
	...
	let menuControl: React.ReactNode;
	if ( loadingMenus ) {
		menuControl = (
			<div
				style={ {
		...
		menuControl = (
			<Notice status="warning" isDismissible={ false }>
				{ __(
					'No navigation menus found. Create one in Appearance → Menus.',
					'goblocks'
				) }
			</Notice>
		);
		...
		menuControl = (
			<SelectControl
				...
				options={ menuOptions }
				onChange={ ( v ) =>
					setAttributes( { menuId: parseInt( v, 10 ) } )
				}
				// @ts-ignore
	...
	const advancedContent = (
		<>
			<PanelBody
				title={ __( 'Navigation Settings', 'goblocks' ) }
				initialOpen
			>
				{ menuControl }
				...
				<SelectControl
					...
					onChange={ ( v ) =>
						setAttributes( { mobileBreakpoint: v } )
					}
					// @ts-ignore
					__nextHasNoMarginBottom
				/>
				<ToggleControl
					...
					onChange={ ( v ) =>
						setAttributes( { showMobileToggle: v } )
					}
					// @ts-ignore
					__nextHasNoMarginBottom
				/>
			...
			<PanelBody
				title={ __( 'Sticky Behavior', 'goblocks' ) }
				initialOpen={ false }
			>
				<ToggleControl
					label={ __( 'Sticky navigation', 'goblocks' ) }
					help={ __(
						'Keep the nav fixed at the top of the viewport while scrolling.',
						'goblocks'
					) }
					checked={ sticky }
				...
				{ sticky && (
					<ToggleControl
						...
						help={ __(
							'Slide the nav out of view when scrolling down; reveal it on scroll up.',
							'goblocks'
						) }
						checked={ scrollHide }
				...
				{ menuId && selectedMenu ? (
					...
					>
						...
						<em style={ { opacity: 0.6 } }>
							{ __( 'Rendered on the frontend.', 'goblocks' ) }
						</em>
					</p>
				) : (
					<p
					...
					>
						{ __(
							'Select a navigation menu in the Settings panel →',
							'goblocks'
						) }
					</p>
				) }
