( function () {
	...
	function buildOverlay(): void {
		...
		overlayCaption =
			overlay.querySelector< HTMLElement >( '.gb-lb__caption' );
		overlayClose =
			overlay.querySelector< HTMLButtonElement >( '.gb-lb__close' );
		overlayPrev =
			overlay.querySelector< HTMLButtonElement >( '.gb-lb__prev' );
		overlayNext =
			overlay.querySelector< HTMLButtonElement >( '.gb-lb__next' );
		overlayCounter =
			overlay.querySelector< HTMLElement >( '.gb-lb__counter' );

		document.body.appendChild( overlay );

		// Close on backdrop click.
		overlay
			.querySelector( '.gb-lb__backdrop' )!
		...
		overlay.addEventListener(
			...
			( e: TouchEvent ) => {
				const dx =
					( e.changedTouches[ 0 ]?.clientX ?? 0 ) - touchStartX;
				if ( Math.abs( dx ) > 50 ) {
					navigate( dx < 0 ? 1 : -1 );
				}
			},
	...
	function updateNav(): void {
		...
		if ( overlayPrev ) {
			overlayPrev.hidden = single;
		}
		if ( overlayNext ) {
			overlayNext.hidden = single;
		}
		if ( overlayCounter ) {
			overlayCounter.textContent = single
				? ''
				: `${ currentIndex + 1 } / ${ count }`;
		}
		if ( overlay ) {
			overlay.classList.toggle( 'gb-lb--single', single );
		}
	}
	...
	function loadImage(
		...
		if ( ! overlay || ! overlayImg || ! overlayCaption ) {
			return;
		}
		...
		const spinner =
			overlay.querySelector< HTMLElement >( '.gb-lb__spinner' );
		if ( spinner ) {
	...
	function openAt( index: number ): void {
		...
		previousFocus = overlay!.ownerDocument.activeElement as HTMLElement;
	...
	function trapFocus( e: KeyboardEvent ): void {
		...
		if ( e.shiftKey && overlay!.ownerDocument.activeElement === first ) {
			e.preventDefault();
			last.focus();
		} else if (
			! e.shiftKey &&
			overlay!.ownerDocument.activeElement === last
		) {
			e.preventDefault();
			first.focus();
		}
