( function () {
	...
	document
		.querySelectorAll< HTMLElement >( '.gb-modal' )
		.forEach( ( modal ) => {
			const trigger =
				modal.querySelector< HTMLElement >( '.gb-modal__trigger' );
			const overlay =
				modal.querySelector< HTMLElement >( '.gb-modal__overlay' );
			const dialog =
				modal.querySelector< HTMLElement >( '.gb-modal__dialog' );
			if ( ! trigger || ! overlay || ! dialog ) {
				return;
			}

			const closeBtn =
				modal.querySelector< HTMLButtonElement >( '.gb-modal__close' );
			const dismissBtn =
			...
			const animation = (
				ALLOWED_ANIMATIONS as readonly string[]
			 ).includes( rawAnimation )
				? rawAnimation
				: 'fade';
			const autoOpen = modal.dataset.autoOpen === 'true';
			const autoDelay = Math.max(
				0,
				parseInt( modal.dataset.autoOpenDelay ?? '0', 10 )
			);
			const cookieName = modal.dataset.cookieName ?? '';
			const cookieDays = Math.max(
				1,
				parseInt( modal.dataset.cookieDays ?? '30', 10 )
			);
			...
			function hasDismissedCookie(): boolean {
				...
				return document.cookie
					.split( ';' )
					.some( ( c ) =>
						c.trim().startsWith( `gb_modal_${ cookieName }=` )
					);
			}
			...
			function setDismissCookie(): void {
				...
				expires.setTime(
					expires.getTime() + cookieDays * 24 * 60 * 60 * 1000
				);
			...
			dialog.addEventListener( 'keydown', ( e ) => {
				...
				if ( e.shiftKey ) {
					if ( dialog.ownerDocument.activeElement === first ) {
						e.preventDefault();
						last!.focus();
					}
				} else if ( dialog.ownerDocument.activeElement === last ) {
					e.preventDefault();
					first!.focus();
				}
			} );
