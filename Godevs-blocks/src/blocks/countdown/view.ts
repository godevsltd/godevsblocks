( function () {
	...
	document
		.querySelectorAll< HTMLElement >( '.gb-countdown' )
		.forEach( ( el ) => {
			...
			function update(): void {
				...
				const daysEl = el.querySelector< HTMLElement >(
					'.gb-countdown__days .gb-countdown__number'
				);
				const hoursEl = el.querySelector< HTMLElement >(
					'.gb-countdown__hours .gb-countdown__number'
				);
				const minutesEl = el.querySelector< HTMLElement >(
					'.gb-countdown__minutes .gb-countdown__number'
				);
				const secondsEl = el.querySelector< HTMLElement >(
					'.gb-countdown__seconds .gb-countdown__number'
				);
				...
				if (
					firstTick ||
					Date.now() - srLastAnnounced >= announceInterval
				) {
					firstTick = false;
