( function () {
	...
	document
		.querySelectorAll< HTMLElement >( '.gb-social-share' )
		.forEach( ( el ) => {
			el.addEventListener( 'click', ( e ) => {
				const btn = ( e.target as HTMLElement ).closest< HTMLElement >(
					'.gb-social-share__btn'
				);
				if ( ! btn ) {
					return;
				}
				...
				if ( copyUrl !== undefined ) {
					...
					if ( navigator.clipboard ) {
						navigator.clipboard
							.writeText( url )
							.then( markCopied )
							.catch( () => fallbackCopy( url, markCopied ) );
					} else {
				...
				if ( btn.dataset.popup === 'true' ) {
					...
					if ( href ) {
						window.open(
							href,
							'gb-share-popup',
							'width=620,height=520,resizable=yes,scrollbars=yes'
						);
					}
				}
			} );
