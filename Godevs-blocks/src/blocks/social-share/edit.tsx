...
const ICON_MAP: Record< string, string > = {
	facebook:
		'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/></svg>',
	twitter:
		'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.747l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>',
	linkedin:
		'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-4 0v7h-4v-7a6 6 0 0 1 6-6z"/><rect x="2" y="9" width="4" height="12"/><circle cx="4" cy="4" r="2"/></svg>',
	whatsapp:
...
export function Edit( {
	...
	useCssEngine( {
		...
		generatedCss,
		setAttributes: ( patch ) =>
			setAttributes( patch as Partial< SocialShareBlockAttributes > ),
	} );
		...
		<>
			...
			>
				<SelectControl
					...
					options={ [
						{
							label: __( 'Filled (platform color)', 'goblocks' ),
							value: 'filled',
						},
						{
						...
						{
							label: __( 'Minimal (text + icon)', 'goblocks' ),
							value: 'minimal',
						},
						{
							label: __( 'Rounded (icon badges)', 'goblocks' ),
							value: 'rounded',
						},
					] }
			...
			>
				<TextControl
					...
					value={ customUrl }
					placeholder={ __(
						'Leave blank to use current page URL',
						'goblocks'
					) }
					onChange={ ( v ) => setAttributes( { customUrl: v } ) }
					help={ __(
						'Override the URL shared by all buttons.',
						'goblocks'
					) }
					// @ts-ignore
					__nextHasNoMarginBottom
					...
					>
						<span
							...
							dangerouslySetInnerHTML={ {
								__html: ICON_MAP[ p.value ] ?? '',
							} }
						/>
						{ showLabels && (
							<span className="gb-social-share__label">
								{ p.label }
							</span>
						) }
					</button>
