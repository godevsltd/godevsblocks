	...
	document
		...
		.forEach( ( tocEl ) => {
			...
			if ( ! tocEl.hasAttribute( 'aria-label' ) ) {
				...
				tocEl.setAttribute(
					'aria-label',
					titleEl?.textContent?.trim() || 'Table of Contents'
				);
			}
			...
			function updateActiveLink( activeId: string | null ): void {
				tocEl
					.querySelectorAll< HTMLAnchorElement >( '.gb-toc__link' )
					.forEach( ( a ) => {
			...
			if ( hasIO ) {
				...
				function updateActive(): void {
					...
					headingEls.forEach( ( h ) => {
						if ( h.getBoundingClientRect().top <= offset + 10 ) {
							active = h;
						}
					} );
					updateActiveLink(
						( active as HTMLElement | null )?.id ?? null
					);
				}
			...
			if ( tocEl.dataset.collapsible === 'true' ) {
				const titleEl =
					tocEl.querySelector< HTMLElement >( '.gb-toc__title' );
				if ( titleEl ) {
					const listId =
						'gb-toc-list-' +
						( tocEl.id || Math.random().toString( 36 ).slice( 2 ) );
					...
					function toggle() {
						const collapsed =
							tocEl.classList.toggle( 'is-collapsed' );
						titleEl!.setAttribute(
							'aria-expanded',
							String( ! collapsed )
						);
