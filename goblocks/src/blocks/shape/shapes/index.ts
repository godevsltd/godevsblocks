...
export function buildShapeSvg(
	...
	const transform =
		flipX || flipY
			? ` transform="scale(${ flipX ? -1 : 1 },${ flipY ? -1 : 1 }) translate(${ flipX ? -shape.width : 0 },${ flipY ? -height : 0 })"`
			: '';
	...
	if ( useGradient ) {
		...
		const x1   = 1 - x2;
		const y1   = 1 - y2;
		defs = `<defs><linearGradient id="${ gradId }" x1="${ x1.toFixed( 4 ) }" y1="${ y1.toFixed( 4 ) }" x2="${ x2.toFixed( 4 ) }" y2="${ y2.toFixed( 4 ) }"><stop offset="0%" stop-color="${ gradientFrom }"/><stop offset="100%" stop-color="${ gradientTo }"/></linearGradient></defs>`;
	...
	const innerHtml = useGradient
		? shape.inner.replace( /fill="currentColor"/g, `fill="url(#${ gradId })"` )
		: shape.inner;
