...
function StarIcon( { fill }: { fill: 'full' | 'half' | 'empty' } ) {
	if ( fill === 'half' ) {
		return (
			<>
				<svg className="gb-star__svg gb-star__bg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" aria-hidden="true">
					<polygon points={ STAR_POINTS } />
				</svg>
				<svg className="gb-star__svg gb-star__fg" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" strokeWidth="1.5" aria-hidden="true">
					<polygon points={ STAR_POINTS } />
				</svg>
			</>
		);
	...
	return (
		<svg className="gb-star__svg" viewBox="0 0 24 24" fill={ fill === 'full' ? 'currentColor' : 'none' } stroke="currentColor" strokeWidth="1.5" aria-hidden="true">
			<polygon points={ STAR_POINTS } />
		</svg>
	);
}
...
export function Edit( {
	...
	const {
		...
		rating, maxRating, showNumber, label,
		starColor, emptyColor, starSize,
		reviewCount, showCount, schemaEnabled, itemName,
		animateOnScroll,
		generatedCss,
	} = attributes;
	...
	useCssEngine( {
		...
		styles,
		generatedCss,
		setAttributes: ( patch ) => setAttributes( patch as Partial< StarRatingBlockAttributes > ),
	} );
	...
	const stars = Array.from( { length: maxRating }, ( _, i ) => {
		const n = i + 1;
		if ( n <= Math.floor( rating ) ) return 'full' as const;
		if ( n - 0.5 <= rating )         return 'half' as const;
	...
	const stylesContent = (
		<>
			<PanelBody title={ __( 'Star Appearance', 'goblocks' ) } initialOpen>
				<RangeControl
				...
				<ColorControl
					...
					onChange={ ( v ) => setAttributes( { starColor: v || '#f59e0b' } ) }
				/>
				<ColorControl
					label={ __( 'Empty Star Color', 'goblocks' ) }
					breakpoint={ responsive.activeBreakpoint }
					value={ emptyColor || '#d1d5db' }
					onChange={ ( v ) => setAttributes( { emptyColor: v || '#d1d5db' } ) }
				/>
				<ToggleControl
					label={ __( 'Animate on scroll', 'goblocks' ) }
					help={ __( 'Stars pop in one by one when scrolled into view.', 'goblocks' ) }
	...
	const advancedContent = (
		<>
			<PanelBody title={ __( 'Star Rating Settings', 'goblocks' ) } initialOpen>
				<RangeControl
			...
			<PanelBody title={ __( 'Review Count', 'goblocks' ) } initialOpen={ false }>
				<NumberControl
					label={ __( 'Number of reviews', 'goblocks' ) }
					value={ reviewCount ?? 1 }
					min={ 1 }
					onChange={ ( v ) => setAttributes( { reviewCount: parseInt( String( v ?? 1 ), 10 ) || 1 } ) }
					help={ __( 'Used for schema.org markup and optional display.', 'goblocks' ) }
					// @ts-ignore
					__next40pxDefaultSize
				/>
				<ToggleControl
					label={ __( 'Show review count', 'goblocks' ) }
					help={ __( 'Displays "(X reviews)" next to the rating.', 'goblocks' ) }
					checked={ showCount }
			...
			<PanelBody title={ __( 'Schema.org Markup', 'goblocks' ) } initialOpen={ false }>
				<ToggleControl
					label={ __( 'Output structured data', 'goblocks' ) }
					help={ __( 'Adds JSON-LD schema markup (AggregateRating) to help search engines display star ratings in results.', 'goblocks' ) }
				...
				{ schemaEnabled && (
					<TextControl
						...
						value={ itemName }
						placeholder={ __( 'Defaults to the page/post title', 'goblocks' ) }
						onChange={ ( v ) => setAttributes( { itemName: v } ) }
						help={ __( 'Name of the product, service, or entity being rated.', 'goblocks' ) }
						// @ts-ignore
						__nextHasNoMarginBottom
					/>
				) }
	...
	return (
		<>
			...
			<div { ...blockProps }>
				{ label && <span className="gb-star-rating__label">{ label }</span> }
				<div
					className="gb-star-rating__stars"
					role="img"
					aria-label={ `${ rating } ${ __( 'out of', 'goblocks' ) } ${ maxRating } ${ __( 'stars', 'goblocks' ) }` }
				>
					{ stars.map( ( fill, i ) => (
						<span key={ i } className={ `gb-star gb-star--${ fill }` }>
							<StarIcon fill={ fill } />
						</span>
					) ) }
				</div>
				{ showNumber && (
					<span className="gb-star-rating__number">{ rating }/{ maxRating }</span>
				) }
				{ showCount && (
					<span className="gb-star-rating__count">
						({ count } { count === 1 ? __( 'review', 'goblocks' ) : __( 'reviews', 'goblocks' ) })
					</span>
				) }
			</div>
		</>
