...
export function Edit( {
	...
	const {
		...
		url, autoplay, muted, loop, controls, playsinline, poster, ratio,
		lazyLoad, youtubeRel, youtubeModestBranding, youtubeStart, vimeoHideBranding,
		caption, videoTitle, preload,
		generatedCss,
	} = attributes;
	...
	useCssEngine( {
		...
		styles,
		generatedCss,
		setAttributes: ( patch ) => setAttributes( patch as Partial< VideoBlockAttributes > ),
	} );
	...
	const advancedContent = (
		<>
			<PanelBody title={ __( 'Video Source', 'goblocks' ) } initialOpen>
				<TextControl
					label={ __( 'Video URL', 'goblocks' ) }
					value={ url }
					onChange={ ( v ) => setAttributes( { url: v } ) }
					placeholder={ __( 'YouTube, Vimeo or .mp4 URL', 'goblocks' ) }
					help={ __( 'Paste a YouTube, Vimeo, or direct video file URL.', 'goblocks' ) }
					// @ts-ignore
				...
				{ isEmbed && (
					<>
						...
						<ToggleControl
							label={ __( 'Lazy load (recommended)', 'goblocks' ) }
							help={ isYouTube
								? __( 'Shows a thumbnail with play button; loads the iframe only on click.', 'goblocks' )
								: __( 'Lazy load applies to YouTube embeds only.', 'goblocks' ) }
							checked={ lazyLoad }
				...
				<TextControl
					...
					help={ __( 'Used as the iframe title and visible caption. Leave blank for default.', 'goblocks' ) }
				...
				<TextControl
					...
					value={ caption }
					placeholder={ __( 'Optional caption shown below the video…', 'goblocks' ) }
			...
			{ isYouTube && (
				<PanelBody title={ __( 'YouTube Settings', 'goblocks' ) } initialOpen={ false }>
					<ToggleControl
						label={ __( 'Hide related videos', 'goblocks' ) }
						help={ __( 'Limits related videos after playback to the same channel (rel=0).', 'goblocks' ) }
						checked={ ! youtubeRel }
						onChange={ ( v ) => setAttributes( { youtubeRel: ! v } ) }
						// @ts-ignore
						__nextHasNoMarginBottom
					/>
					<ToggleControl
						label={ __( 'Modest branding', 'goblocks' ) }
						help={ __( 'Minimises the YouTube logo in the player.', 'goblocks' ) }
						checked={ youtubeModestBranding }
						onChange={ ( v ) => setAttributes( { youtubeModestBranding: v } ) }
						// @ts-ignore
						__nextHasNoMarginBottom
					/>
					<div style={ { height: '8px' } } />
					<NumberControl
						...
						min={ 0 }
						onChange={ ( v ) => setAttributes( { youtubeStart: parseInt( String( v ?? 0 ), 10 ) || 0 } ) }
						help={ __( 'Video begins at this offset. Leave 0 to start from the beginning.', 'goblocks' ) }
						// @ts-ignore
			...
			{ isVimeo && (
				<PanelBody title={ __( 'Vimeo Settings', 'goblocks' ) } initialOpen={ false }>
					<ToggleControl
						label={ __( 'Hide branding', 'goblocks' ) }
						help={ __( 'Hides the Vimeo title, byline, and portrait from the player.', 'goblocks' ) }
						checked={ vimeoHideBranding }
						onChange={ ( v ) => setAttributes( { vimeoHideBranding: v } ) }
						// @ts-ignore
						__nextHasNoMarginBottom
					/>
				</PanelBody>
			...
			<PanelBody title={ __( 'Playback Options', 'goblocks' ) } initialOpen={ false }>
				<ToggleControl
					label={ __( 'Autoplay', 'goblocks' ) }
					help={ __( 'Automatically enables Muted (required by browsers).', 'goblocks' ) }
					checked={ autoplay }
				...
				{ isSelfHosted && (
					<>
						...
						<SelectControl
							...
							options={ [
								{ label: __( 'Metadata only (recommended)', 'goblocks' ), value: 'metadata' },
								{ label: __( 'None — load on play',          'goblocks' ), value: 'none'     },
								{ label: __( 'Auto — preload entire video',  'goblocks' ), value: 'auto'     },
							] }
							onChange={ ( v ) => setAttributes( { preload: v } ) }
							help={ __( 'Controls how much data the browser fetches before the user plays.', 'goblocks' ) }
							// @ts-ignore
							__nextHasNoMarginBottom
						/>
					</>
				...
				<div { ...blockProps }>
					...
					{ ! url && (
						<div className="gb-video__placeholder">
							<span>{ __( 'Enter a video URL in the Advanced panel.', 'goblocks' ) }</span>
						</div>
					) }
				</div>
				{ caption && (
					<figcaption className="gb-video__caption">{ caption }</figcaption>
				) }
			</figure>
		</>
	);
