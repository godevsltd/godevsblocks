( function () {
	...
	document
		.querySelectorAll< HTMLElement >( '.gb-timeline' )
		.forEach( ( timeline ) => {
			const items = Array.from(
				timeline.querySelectorAll< HTMLElement >(
					':scope > .gb-timeline-item'
				)
			);
			if ( ! items.length ) {
				return;
			...
			const isAlternating = timeline.classList.contains(
				'gb-timeline--alternating'
			);
			...
			if ( animation === 'slide' && isAlternating ) {
				items.forEach( ( item, i ) => {
					item.classList.add(
						i % 2 === 0 ? 'gb-tl-from-left' : 'gb-tl-from-right'
					);
				} );
			}
