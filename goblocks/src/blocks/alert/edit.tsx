...
const STYLE_OPTIONS = [
	{
		...
		preview: (
			<svg width="56" height="30" viewBox="0 0 56 30" fill="none" aria-hidden="true">
				<rect x="0" y="0" width="56" height="30" rx="5" fill="#eff6ff"/>
				<rect x="0" y="0" width="4"  height="30" rx="2" fill="#1d4ed8"/>
				<rect x="10" y="7"  width="20" height="4" rx="2" fill="#1d4ed8"/>
				<rect x="10" y="15" width="32" height="3" rx="1.5" fill="#1d4ed8" opacity="0.5"/>
			</svg>
		),
	},
	{
		...
		preview: (
			<svg width="56" height="30" viewBox="0 0 56 30" fill="none" aria-hidden="true">
				<rect x="1" y="1" width="54" height="28" rx="5" stroke="#1d4ed8" strokeWidth="1.5"/>
				<rect x="9"  y="7"  width="20" height="4" rx="2" fill="#1d4ed8"/>
				<rect x="9"  y="15" width="32" height="3" rx="1.5" fill="#1d4ed8" opacity="0.5"/>
			</svg>
		),
	},
	{
...
export function Edit( {
	...
	const {
		...
		title, message,
		dismissible, dismissMode, dismissDays,
		icon: showIcon,
		sticky,
		generatedCss,
	} = attributes;
	...
	useCssEngine( {
		...
		styles,
		generatedCss,
		setAttributes: ( patch ) => setAttributes( patch as Partial< AlertBlockAttributes > ),
	} );
	...
	const blockProps = useBlockProps( { className: wrapperClass, role: 'alert' } );
	...
	const advancedContent = (
		<>
			<PanelBody title={ __( 'Alert Settings', 'goblocks' ) } initialOpen>
				<SelectControl
					...
					options={ [
						...
						{ label: __( 'Success', 'goblocks' ), value: 'success' },
						{ label: __( 'Warning', 'goblocks' ), value: 'warning' },
				...
				<ToggleControl
					...
					help={ __( 'Fix the alert to the top of the viewport (useful for site-wide notices).', 'goblocks' ) }
					checked={ sticky }
			...
			<PanelBody title={ __( 'Dismiss', 'goblocks' ) } initialOpen={ false }>
				<ToggleControl
					label={ __( 'Dismissible', 'goblocks' ) }
					help={ __( 'Show a close button on the alert.', 'goblocks' ) }
					checked={ dismissible }
				...
				{ dismissible && (
					<>
						<SelectControl
							...
							help={ __( 'How long to remember that this alert was dismissed.', 'goblocks' ) }
							value={ dismissMode ?? 'none' }
							options={ [
								{ label: __( 'Never (show again on reload)',  'goblocks' ), value: 'none'    },
								{ label: __( 'Until tab is closed (session)', 'goblocks' ), value: 'session' },
								{ label: __( 'Forever (localStorage)',        'goblocks' ), value: 'local'   },
								{ label: __( 'For N days (cookie)',           'goblocks' ), value: 'cookie'  },
							] }
							onChange={ ( v ) => setAttributes( { dismissMode: v } ) }
							// @ts-ignore
							__nextHasNoMarginBottom
						/>
						{ dismissMode === 'cookie' && (
							<NumberControl
								...
								max={ 365 }
								onChange={ ( v ) =>
									setAttributes( { dismissDays: Math.max( 1, parseInt( String( v ?? 30 ), 10 ) || 30 ) } )
								}
								// @ts-ignore
		...
		<>
			...
			<div { ...blockProps }>
				...
				<div className="gb-alert__body">
					{ title && <strong className="gb-alert__title">{ title }</strong> }
					<RichText
						tagName="p"
				...
				{ dismissible && (
					...
					>
						<svg viewBox="0 0 20 20" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" aria-hidden="true">
