...
const STYLE_OPTIONS: StyleOption[] = [
	{
		...
		preview: (
			<svg width="60" height="28" viewBox="0 0 60 28" fill="none" aria-hidden="true">
				<rect x="1" y="4" width="22" height="20" rx="10" fill="#4f46e5"/>
				<rect x="27" y="4" width="16" height="20" rx="8" fill="#f3f4f6"/>
				<rect x="47" y="4" width="12" height="20" rx="6" fill="#f3f4f6"/>
			</svg>
		),
	},
	{
		...
		preview: (
			<svg width="60" height="28" viewBox="0 0 60 28" fill="none" aria-hidden="true">
				<text x="1"  y="18" fontSize="11" fill="#4f46e5" fontWeight="700">Tab 1</text>
				<text x="35" y="18" fontSize="11" fill="#9ca3af">Tab 2</text>
				<rect x="1" y="22" width="30" height="2.5" rx="1.25" fill="#4f46e5"/>
			</svg>
		),
	},
	{
		...
		preview: (
			<svg width="60" height="28" viewBox="0 0 60 28" fill="none" aria-hidden="true">
				<rect x="1" y="2" width="24" height="18" rx="4 4 0 0" fill="white" stroke="#d1d5db" strokeWidth="1.5"/>
				<rect x="28" y="5" width="20" height="15" rx="4 4 0 0" fill="#f9fafb" stroke="#e5e7eb" strokeWidth="1.5"/>
				<line x1="0" y1="20" x2="60" y2="20" stroke="#d1d5db" strokeWidth="1.5"/>
			</svg>
		),
	},
	{
		...
		preview: (
			<svg width="60" height="28" viewBox="0 0 60 28" fill="none" aria-hidden="true">
				<rect x="1" y="2" width="58" height="24" rx="8" fill="#f1f5f9"/>
				<rect x="4" y="5" width="22" height="18" rx="6" fill="white"/>
				<rect x="29" y="5" width="14" height="18" rx="5" fill="transparent"/>
				<rect x="46" y="5" width="10" height="18" rx="5" fill="transparent"/>
			</svg>
		),
	},
];
...
export function TabsInspector( {
	...
	const { orientation, tabStyle, tabsFullWidth, defaultTab, globalClasses, styles } = attributes;

	const responsive = useResponsiveStyles( styles, ( patch ) =>
		setAttributes( { styles: patch.styles as BlockStyles } )
	);

	const activeColor = responsive.getStyle( 'variables', '--gb-tabs-active-color' ) ?? '#4f46e5';
	const activeText  = responsive.getStyle( 'variables', '--gb-tabs-active-text'  ) ?? '#ffffff';
	const btnColor    = responsive.getStyle( 'variables', '--gb-tabs-btn-color'    ) ?? '#6b7280';

	const stylesContent = (
		<>
			{ /* ── Style variant ──────────────────────────────────────────── */ }
			<PanelBody title={ __( 'Tab Style', 'goblocks' ) } initialOpen>
				<StylePicker value={ tabStyle ?? 'pill' } onChange={ ( v ) => setAttributes( { tabStyle: v } ) } />
				<ToggleControl
					label={ __( 'Full-width tabs', 'goblocks' ) }
					help={ __( 'Tabs stretch to fill the full bar width.', 'goblocks' ) }
			...
			<PanelBody title={ __( 'Colors', 'goblocks' ) } initialOpen={ false }>
				{ /* Live mini-preview */ }
				<div style={ { display: 'flex', gap: '6px', padding: '12px', background: '#f8fafc', borderRadius: '8px', marginBottom: '12px', flexWrap: 'wrap' } }>
				...
				<ColorControl
					...
					value={ activeColor }
					onChange={ ( val ) => responsive.setStyle( 'variables', '--gb-tabs-active-color', val ) }
				/>
				...
				<ColorControl
					...
					breakpoint={ responsive.activeBreakpoint }
					value={ activeText }
					onChange={ ( val ) => responsive.setStyle( 'variables', '--gb-tabs-active-text', val ) }
				/>
				<div style={ { height: '12px' } } />
				<ColorControl
					...
					breakpoint={ responsive.activeBreakpoint }
					value={ btnColor }
					onChange={ ( val ) => responsive.setStyle( 'variables', '--gb-tabs-btn-color', val ) }
				/>
			</PanelBody>
		...
		<>
			<PanelBody title={ __( 'Tabs Settings', 'goblocks' ) } initialOpen>
				<RadioControl
					...
					onChange={ ( v ) => setAttributes( { orientation: v as 'horizontal' | 'vertical' } ) }
				/>
				<NumberControl
					label={ __( 'Default Active Tab', 'goblocks' ) }
					help={ __( 'Which tab is open by default (1 = first tab).', 'goblocks' ) }
					value={ defaultTab + 1 }
					min={ 1 }
					onChange={ ( v ) => {
						const display = parseInt( String( v ?? 1 ), 10 ) || 1;
						setAttributes( { defaultTab: Math.max( 0, display - 1 ) } );
					} }
					// @ts-ignore
