( function () {
	...
	document.querySelectorAll< HTMLElement >( '.gb-flip-card' ).forEach( ( card ) => {
		const inner     = card.querySelector< HTMLElement >( '.gb-flip-card__inner' );
		...
		if ( onClick ) {
			...
			card.setAttribute( 'aria-label',
				card.dataset.ariaLabel ?? 'Flip card — press to reveal the back side'
			);
		...
		const cta = card.querySelector< HTMLAnchorElement >( '.gb-flip-card__cta' );
		if ( cta ) {
