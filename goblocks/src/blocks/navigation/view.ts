( function () {
	...
	document.querySelectorAll< HTMLElement >( '.gb-navigation' ).forEach( ( nav ) => {
		const toggle = nav.querySelector< HTMLButtonElement >( '.gb-navigation__toggle' );
		const menu   = nav.querySelector< HTMLElement >( '.gb-navigation__menu' );
		...
		const subToggles = Array.from(
			nav.querySelectorAll< HTMLButtonElement >( '.gb-navigation__submenu-toggle' )
		);

		subToggles.forEach( ( btn ) => {
			const li      = btn.closest< HTMLLIElement >( 'li.menu-item-has-children' );
			...
			btn.addEventListener( 'click', ( e ) => {
				e.stopPropagation();
				li.classList.contains( 'is-open' ) ? closeSub() : openSub();
			} );

			// Desktop hover (only when not in mobile mode).
			li.addEventListener( 'mouseenter', () => {
				if ( ! isMobile( nav ) ) openSub();
			} );
			li.addEventListener( 'mouseleave', () => {
				if ( ! isMobile( nav ) ) closeSub();
			} );
			...
			document.addEventListener( 'click', ( e: MouseEvent ) => {
				if ( ! li.contains( e.target as Node ) ) closeSub();
			} );
		} );

		// Keyboard arrow navigation through top-level items.
		const topItems = Array.from(
			nav.querySelectorAll< HTMLAnchorElement >( '.gb-navigation__menu > li > a' )
		);
		topItems.forEach( ( a, i ) => {
			a.addEventListener( 'keydown', ( e: KeyboardEvent ) => {
				...
				if ( e.key === 'ArrowLeft' || e.key === 'ArrowUp' ) {
					e.preventDefault();
					topItems[ ( i - 1 + topItems.length ) % topItems.length ]?.focus();
				}
			} );
		} );
