( function () {
	...
	document.querySelectorAll< HTMLElement >( '.gb-slider' ).forEach( ( slider ) => {
		const track          = slider.querySelector< HTMLElement >( '.gb-slider__track' );
		const prevBtn        = slider.querySelector< HTMLButtonElement >( '.gb-slider__prev' );
		const nextBtn        = slider.querySelector< HTMLButtonElement >( '.gb-slider__next' );
		const pauseBtn       = slider.querySelector< HTMLButtonElement >( '.gb-slider__pause' );
		const dotsEl         = slider.querySelector< HTMLElement >( '.gb-slider__dots' );
		const counterCurrent = slider.querySelector< HTMLElement >( '.gb-slider__counter-current' );
		const counterTotal   = slider.querySelector< HTMLElement >( '.gb-slider__counter-total' );
		const progressBar    = slider.querySelector< HTMLElement >( '.gb-slider__progress-bar' );
		...
		const spv          = Math.max( 1, parseInt( slider.dataset.spv   || '1',    10 ) || 1 );
		const slideGap     = Math.max( 0, parseInt( slider.dataset.gap   || '0',    10 ) || 0 );
		...
		const dur          = reducedMotion ? 0 : parseInt( slider.dataset.duration || '450', 10 );
		const ease         = reducedMotion ? 'linear' : ( slider.dataset.easing || 'ease' );
		...
		const maxIndex = effect === 'slide' && spv > 1
		...
		if ( effect === 'fade' ) {
			...
			if ( slideGap > 0 ) {
				...
				const sw = spv > 1
					...
					: avail;
				slides.forEach( ( s ) => { s.style.flex = `0 0 ${ sw }px`; } );
			} else {
				slides.forEach( ( s ) => { s.style.flex = `0 0 ${ 100 / spv }%`; } );
			}
		}
		...
		if ( dotsEl ) {
			...
			for ( let i = 0; i <= maxIndex; i++ ) {
				...
				dot.addEventListener( 'click', ( () => {
					const idx = i;
					return () => { goTo( idx ); resetTimer(); resetProgress(); };
				} )() );
		...
		if ( prevBtn ) {
			prevBtn.addEventListener( 'click', () => {
				if ( ! dragging ) { goTo( current - 1 ); resetTimer(); resetProgress(); }
			} );
		}
		if ( nextBtn ) {
		...
		slider.addEventListener( 'keydown', ( e: KeyboardEvent ) => {
			switch ( e.key ) {
				case 'ArrowLeft':
					e.preventDefault(); goTo( current - 1 ); resetTimer(); resetProgress(); break;
				case 'ArrowRight':
					e.preventDefault(); goTo( current + 1 ); resetTimer(); resetProgress(); break;
				case 'Home':
					e.preventDefault(); goTo( 0 ); resetTimer(); resetProgress(); break;
				case 'End':
					e.preventDefault(); goTo( maxIndex ); resetTimer(); resetProgress(); break;
			}
		} );
		...
		function resetTimer(): void {
			stopTimer();
			if ( autoplay && ! paused && inView ) startTimer();
		}
		...
		if ( autoplay ) {
			if ( pauseOnHover ) {
				...
				slider.addEventListener( 'mouseleave', () => {
					paused = false;
					if ( inView ) { startTimer(); startProgress(); }
				} );
