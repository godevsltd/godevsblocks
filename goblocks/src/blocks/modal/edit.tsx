...
export function Edit( {
	...
	useCssEngine( {
		...
		generatedCss,
		setAttributes: ( patch ) =>
			setAttributes( patch as Partial< ModalBlockAttributes > ),
	} );
		...
		<>
			...
			>
				<ToggleControl
					...
					help={ __(
						'Use with a cookie to avoid showing repeatedly.',
						'goblocks'
					) }
					checked={ autoOpen }
				...
				{ autoOpen && (
					<>
						...
						<RangeControl
							...
							onChange={ ( v ) =>
								setAttributes( { autoOpenDelay: v ?? 2000 } )
							}
							min={ 0 }
							max={ 10000 }
							step={ 500 }
							help={ __(
								'How long to wait after page load before opening.',
								'goblocks'
							) }
							// @ts-ignore
							__nextHasNoMarginBottom
			...
			<PanelBody
				title={ __( '"Don\'t show again" Cookie', 'goblocks' ) }
				initialOpen={ false }
			>
				<TextControl
					...
					onChange={ ( v ) =>
						setAttributes( {
							cookieName: v.replace( /[^a-z0-9_-]/gi, '' ),
						} )
					}
					help={ __(
						'Unique identifier. When set, the "Don\'t show again" button stores a cookie to suppress auto-open.',
						'goblocks'
					) }
					// @ts-ignore
					__nextHasNoMarginBottom
				...
				{ cookieName && (
					<>
						...
						<ToggleControl
							...
							help={ __(
								'Renders a dismiss button inside the modal footer.',
								'goblocks'
							) }
							checked={ showDismissBtn }
							onChange={ ( v ) =>
								setAttributes( { showDismissBtn: v } )
							}
							// @ts-ignore
							__nextHasNoMarginBottom
						/>
	...
	return (
		<>
			...
			<div { ...blockProps }>
				{ triggerType === 'button' ? (
					<button className="gb-modal__trigger">
						{ triggerText }
					</button>
				) : (
					<a className="gb-modal__trigger" href="#modal">
						{ triggerText }
					</a>
				) }
				{ /* Always-visible dialog preview in the editor */ }
				<div
					className="gb-modal__overlay"
					aria-hidden="true"
					style={ { display: 'block', position: 'relative' } }
				>
					<div
						className="gb-modal__dialog"
						role="dialog"
						aria-modal="true"
					>
						<button
							className="gb-modal__close"
							aria-label={ __( 'Close', 'goblocks' ) }
						>
							&times;
						</button>
