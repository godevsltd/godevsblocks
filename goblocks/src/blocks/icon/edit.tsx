...
function buildCssVars( attrs: IconBlockAttributes ): React.CSSProperties {
	...
	if ( attrs.iconColor ) {
		vars[ '--gb-icon-color' ] = attrs.iconColor;
	}
	if ( attrs.iconHoverColor ) {
		vars[ '--gb-icon-hover' ] = attrs.iconHoverColor;
	}
	if ( attrs.iconBgColor ) {
		vars[ '--gb-icon-bg' ] = attrs.iconBgColor;
	}
	if ( attrs.iconBgHoverColor ) {
		vars[ '--gb-icon-bg-hover' ] = attrs.iconBgHoverColor;
	}
	if ( attrs.iconBgPadding ) {
		vars[ '--gb-icon-bg-pad' ] = `${ attrs.iconBgPadding }px`;
	}
...
export function Edit( {
	...
	const wrapperClass = clsx(
		...
		hasAnim &&
			...
			`gb-icon--trigger-${ animationTrigger }`,
		...( globalClasses ?? [] )
	);
